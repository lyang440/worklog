...
(function() {
  angular.module('app.controller').controller('LayoutController', ['$rootScope', '$scope', '$window', 'homeService', 'modalService', function($rootScope, $scope, $window, homeService, modalService) {
    ...
    $scope.$watch('date', function() {
      return $scope.$broadcast('dateSelecteded', $scope.date);
    });
    $scope.open = function(event) {
      event.preventDefault();
      event.stopPropagation();
    ...
    $scope.getDepartments = function() {
      return homeService.list().then(function(departmentData) {
        return $scope.departments = departmentData;
      }, function(err) {
        return notificationService.error('刷新错误', '提示');
      });
    };
    $scope.departmentSelected = function(name) {
      return $scope.departmentName = name;
    };
    $scope.memberSelected = function(name) {
      return $scope.memberName = name;
    };
    $scope.addMyWorkLog = function() {
      var opt;
...
(function() {
  angular.module('app.controller').controller('homeController', ['$rootScope', '$scope', '$window', 'homeService', 'modalService', 'notificationService', function($rootScope, $scope, $window, homeService, modalService, notificationService) {
    ...
    $scope.date = new Date();
    $scope.department = $scope.$parent.departmentName ? $scope.$parent.departmentName : '';
    $scope.member = $scope.$parent.memberName ? $scope.$parent.memberName : '';
    $scope.$on('dateSelecteded', function(e, result) {
      $scope.date = result;
      return $scope.getDepartmentWorkLogList();
    });
    $scope.getDepartmentWorkLogList = function() {
      return homeService.listByDate($scope.date).then(function(workLogList) {
        var workLog, _i, _len;
        $scope.workLogList = workLogList;
        for (_i = 0, _len = workLogList.length; _i < _len; _i++) {
          workLog = workLogList[_i];
          $scope.mood_lovely;
          $scope.mood_smile;
          $scope.mood_sad;
          $scope.mood_pain;
        }
        return console.log(workLogList);
      }, function(err) {
        return notificationService.error('刷新错误', '提示');
      });
    };
